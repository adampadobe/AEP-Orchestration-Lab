...
async function main() {
  ...
  const blocked = extracted.title === 'Access Denied' || extracted.stories.length === 0;
  let payload;

  if (blocked) {
    const curatedPath = path.join(ROOT, 'scrape-data.json');
    if (fs.existsSync(curatedPath)) {
      payload = JSON.parse(fs.readFileSync(curatedPath, 'utf8'));
      payload.scrape_id = scrapeId;
      payload.scrapedAt = new Date().toISOString();
      payload.scrapeMethod = 'playwright_blocked_fallback_to_curated';
    } else {
      payload = {
        scrape_id: scrapeId,
        scrapedAt: new Date().toISOString(),
        sourceUrl: 'https://news.sky.com/uk',
        scrapeMethod: 'playwright_blocked_no_curated',
        ...extracted,
      };
    }
  } else {
