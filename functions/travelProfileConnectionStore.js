/**
 * Persist AEP Lab Travel Profile streaming + infra IDs per Adobe sandbox (Firestore).
 * Separate collection from `genericProfileConnections` and `consentConnections`
 * so each industry/flow keeps its own (schema, dataset, dataflow) triplet
 * without colliding on a shared Firestore doc.
 *
