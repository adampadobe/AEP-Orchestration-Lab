/**
 * Persist AEP Lab FSI Profile streaming + infra IDs per Adobe sandbox
 * (Firestore). Thin config wrapper around `profileConnectionStoreFactory`.
 *
 * Separate Firestore collection from each other industry's `*ProfileConnections`
 * so the (schema, dataset, dataflow) triplets never collide in a shared sandbox.
 *
...
const COLLECTION = 'fsiProfileConnections';
