...
async function saveScrape(sandbox, payload, options = {}) {
  ...
  const indexDoc = {
    ...
    demoWebsitePath: (fullRecord.demoWebsite && (fullRecord.demoWebsite.path || fullRecord.demoWebsite.publicUrl)) || null,
    profileViewerDemoHref: (fullRecord.demoWebsite && fullRecord.demoWebsite.profileViewerDemoHref) || null,
...
async function listScrapes(sandbox, { limit = 50 } = {}) {
  ...
  snap.forEach((d) => {
    ...
    items.push(hydrate({
      ...
      customerLogoUrl: data.customerLogoUrl || null,
