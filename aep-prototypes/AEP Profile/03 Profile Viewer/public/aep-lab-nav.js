...
(function () {
  ...
  /** Bind Firebase auth visibility updates only once, even when deferred dashboards remount. */
  var ownerVisibilityAuthBound = false;

  function normalizePortalUserEmail(value) {
    return String(value || '').trim().toLowerCase();
  }

  /**
   * Read the signed-in Firebase email synchronously so owner-only navigation
   * remains hidden by default and works on older pages that do not load the
   * Firebase SDK themselves.
   */
  function currentPortalUserEmails() {
    var seen = {};
    var emails = [];
    function add(value) {
      var email = normalizePortalUserEmail(value);
  ...
  function isUserAllowedForNavItem(item) {
    if (!item || !item.onlyUserEmails) return true;
    var allow = Array.isArray(item.onlyUserEmails) ? item.onlyUserEmails : [item.onlyUserEmails];
    var allowed = allow.map(normalizePortalUserEmail).filter(Boolean);
    if (!allowed.length) return false;
    return currentPortalUserEmails().some(function (email) {
      return allowed.indexOf(email) !== -1;
    });
  }

  function applyOwnerOnlyElementVisibility() {
    try {
      document.querySelectorAll('[data-aep-owner-emails]').forEach(function (element) {
        var allow = String(element.getAttribute('data-aep-owner-emails') || '')
          .split(',')
          .map(normalizePortalUserEmail)
          .filter(Boolean);
        var visible = currentPortalUserEmails().some(function (email) {
  ...
  function shouldShowNavItem(item) {
    if (!isUserAllowedForNavItem(item)) return false;
  ...
  var NAV = [
    ...
    },
    {
      ...
      items: [
        ...
        { label: 'Orchestrated campaign triggers', href: 'orchestrated-campaign-trigger.html', ico: '<svg width="16" height="16" viewBox="0 0 20 20" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M3 4h9M3 10h7M3 16h5"/><path d="M13 8l4 2-4 2V8z"/><circle cx="16" cy="4" r="2"/></svg>' },
        ...
        { label: 'PDF conversion', href: 'pdf-personalisation.html', allowWorkspace: true, ico: '<svg width="16" height="16" viewBox="0 0 20 20" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M4 2.5h8l4 4V17a1 1 0 0 1-1 1H4a1 1 0 0 1-1-1V3.5a1 1 0 0 1 1-1z"/><path d="M12 2.5v4h4M6.5 10h7M6.5 13h5"/><path d="M14.8 14.8l.4 1.1 1.1.4-1.1.4-.4 1.1-.4-1.1-1.1-.4 1.1-.4z" fill="currentColor" stroke="none"/></svg>' },
        ...
        { label: 'Loyalty reward provider', href: 'loyalty-reward-provider.html', ico: '<svg width="16" height="16" viewBox="0 0 20 20" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M10 2.5l1.2 3.6h3.8l-3.1 2.2 1.2 3.6L10 9.7 6.9 12l1.2-3.6-3.1-2.2h3.8z"/><rect x="3" y="14" width="14" height="3.5" rx="1"/></svg>' },
        ...
        { label: 'MCP servers', href: 'mcp-servers.html', allowWorkspace: true, ico: '<svg width="16" height="16" viewBox="0 0 20 20" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><rect x="3" y="3" width="6" height="6" rx="1"/><rect x="11" y="3" width="6" height="6" rx="1"/><rect x="3" y="11" width="6" height="6" rx="1"/><path d="M11 14h6M14 11v6"/><circle cx="6" cy="6" r="1" fill="currentColor" stroke="none"/></svg>' },
        { label: 'Claude skills', href: 'claude-skills.html', allowWorkspace: true, ico: '<svg width="16" height="16" viewBox="0 0 20 20" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><rect x="2.5" y="3" width="15" height="14" rx="1.5"/><path d="M6 7h8M6 10h5M6 13h6"/><circle cx="14.5" cy="10" r="2.5"/></svg>' },
      ],
    },
    {
      group: 'Decisioning', id: 'decisioning',
      items: [
        { label: 'Decisioning lab (in development)', href: 'content-decision-live.html', inDevelopment: true, navHideKey: 'decisioningLab', onlyUserEmails: ['apalmer@adobe.com'], ico: '<svg width="16" height="16" viewBox="0 0 20 20" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><rect x="1" y="3" width="5" height="4" rx="1"/><rect x="1" y="13" width="5" height="4" rx="1"/><rect x="14" y="8" width="5" height="4" rx="1"/><path d="M6 5h3l3 5.5"/><path d="M6 15h3l3-5"/><path d="M12 10.5h2"/><path d="M10.5 1l.5 1.2.5-1.2M11.7 1.5l-1.2.5 1.2.5" stroke-width="1.1"/></svg>' },
    ...
    },
    {
      ...
      items: [
        {
          label: 'EDS Demo Creator (in development)',
          href: 'eds-quickstart.html',
          inDevelopment: true,
          navHideKey: 'edsQuickstart',
          ico:
          ...
          channels: [
            {
              ...
              items: [
                ...
                },
                {
                  ...
                  demoMeta: { owners: ['apalmer'] },
                  ico:
                    '<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><rect x="3" y="5" width="18" height="14" rx="2"/><path d="m3 7 9 6 9-6"/></svg>',
                },
              ],
            },
            {
  ...
  function getMenuVisibilityOptions() {
    ...
    function pushOption(item, groupName) {
      if (!item) return;
      if (!isUserAllowedForNavItem(item)) return;
  ...
  function buildSidebar(sidebar) {
    applyOwnerOnlyElementVisibility();
  ...
  function init() {
    applyOwnerOnlyElementVisibility();
    if (!ownerVisibilityAuthBound) {
      try {
        if (window.firebase && typeof window.firebase.auth === 'function') {
          ownerVisibilityAuthBound = true;
  ...
  try {
    window.addEventListener('storage', function (e) {
      ...
      if (
        ...
        e.key.indexOf('firebase:authUser:') === 0
      ) {
        applyOwnerOnlyElementVisibility();
        document.querySelectorAll('.dashboard-sidebar').forEach(buildSidebar);
      }
  ...
  try {
    window.AepNavInDev = {
      ...
      currentPortalUserEmails: currentPortalUserEmails,
      isUserAllowedForNavItem: isUserAllowedForNavItem,
