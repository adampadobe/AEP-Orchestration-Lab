/**
 * Local Embed BC response engine (fallback when ?embedBcLocal=1 or Edge errors).
 */
(function (global) {
  const CATALOG_URL = '/embed-bc/army-role-catalog.json';
  const CONFIG_URL = '/embed-bc/embed-bc-agent-config.json';
  ...
  function extractUserMessage(payload) {
    ...
    if (global.__embedBcLastUserText) return global.__embedBcLastUserText;
  ...
  function wireMultimodalCardLinks(root) {
    ...
    scope.querySelectorAll('.bc-multimodal-card').forEach(function (card) {
      if (card.querySelector('a.embed-bc-card-link')) return;
      const label =
      ...
      a.className = 'embed-bc-card-link';
  ...
  function observeMultimodalCards() {
    if (global.__embedBcCardLinkObs) return;
    const obs = new MutationObserver(function () {
      wireMultimodalCardLinks();
    });
    obs.observe(document.documentElement, { childList: true, subtree: true });
    global.__embedBcCardLinkObs = obs;
  }
