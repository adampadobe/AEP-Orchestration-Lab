 ...
 * Loaded by the snapshot page; activated by site-clone-bc.js when Injected is checked.
...
(function (global) {
  ...
  var BASE = PROFILE_VIEWER_PREFIX + 'embed-bc/';
  ...
  var DEFAULT_MOUNT_SELECTOR = '#siteCloneBcInline #brand-concierge-mount';
  ...
  function loadScript(src, doc, marker) {
    var key = String(src);
    var attr = marker ? 'data-site-clone-injected-' + marker : 'data-site-clone-injected';
    ...
    return new Promise(function (resolve, reject) {
      ...
      s.src = key;
      s.async = false;
      s.setAttribute('data-site-clone-injected', key);
  ...
  async function ensureEdgePathPatches(win, doc) {
    ...
    await loadScript(BASE + 'embed-bc-edge-path.js', doc, 'edge-path');
    if (typeof win.applyArmyBcEdgePathPatches === 'function') {
      win.applyArmyBcEdgePathPatches(win);
    }
  }
  ...
  async function ensureCore(doc, win, styleUrl, datastreamId) {
    ...
    if (corePromise && loadedStyleUrl && loadedStyleUrl !== resolvedStyle) {
      global.resetSiteCloneInjectedBc();
    }
    if (
      corePromise &&
      win.__siteCloneBcLoadedDatastreamId &&
      win.__siteCloneBcLoadedDatastreamId !== resolvedDatastream
    ) {
      global.resetSiteCloneInjectedBc();
    }
    if (corePromise) {
    ...
    corePromise = (async function () {
      ...
      await loadScript(BASE + 'embed-bc-scroll-fix.js', doc, 'scroll-fix');
      await loadScript(BASE + 'embed-bc-disclaimer-layout.js', doc, 'disclaimer');
      if (shouldUseLocalCatalog()) {
        await loadScript(BASE + 'embed-bc-local-engine.js', doc, 'local-engine');
        await loadScript(BASE + 'embed-bc-local-fallback.js', doc, 'local-fallback');
      }
  ...
  global.resetSiteCloneInjectedBc = function () {
    corePromise = null;
    loadedStyleUrl = null;
    global.__siteCloneBcBootstrapped = false;
    global.__siteCloneBcAlloyConfiguredWin = null;
    global.__siteCloneBcLoadedDatastreamId = null;
  };
  ...
  function resolveDatastreamId(override) {
    ...
    try {
      if (
        ...
        global.parent.SiteCloneBcConfig &&
        typeof global.parent.SiteCloneBcConfig.getDatastreamId === 'function'
      ) {
        return global.parent.SiteCloneBcConfig.getDatastreamId();
      }
    } catch (_e) {
      /* noop */
    }
  ...
  global.activateSiteCloneInjectedBc = async function (styleUrl, mountSelector, datastreamId) {
    ...
    if (loadedStyleUrl && corePromise) {
      ...
      var dsStored = win.__siteCloneBcLoadedDatastreamId;
    ...
    if (!mount.__siteCloneBcInjectedRepatchBound) {
      mount.__siteCloneBcInjectedRepatchBound = true;
