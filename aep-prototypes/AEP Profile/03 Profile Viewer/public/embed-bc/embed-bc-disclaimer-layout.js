  ...
  function scan(root) {
    ...
    var mounts = scope.querySelectorAll(
      root && root.querySelector
        ? '#brand-concierge-mount, #siteCloneBcModalMount, #siteCloneBcFrameMount, #bcBottomDockMount, #bcModalBarMount'
        : MOUNT_SELECTOR,
    );
    var done = true;
  ...
  global.repositionArmyBcDisclaimer = function (mountOrRoot) {
    if (mountOrRoot && mountOrRoot.nodeType === 1) {
      ...
      if (
        ...
        el.id === 'bcModalBarMount'
      ) {
        if (el.id === 'bcBottomDockMount' || el.id === 'bcModalBarMount') {
