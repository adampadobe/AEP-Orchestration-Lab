...
| **Browser ECID → UPS** | After Launch `getIdentity`, **`navigator.global.browser.profileSeed`** is sent once per tab (synthetic email `navigator-browser-{ecidTail}@example.net` + ECID) so Unified Profile can gain an entity after ingestion. ECID-only lookup may still fail until streaming lands (~1–2 min). |
