  ...
  const MCP_SERVERS = [
    {
      ...
      summary:
        'Lab-hosted Streamable HTTP MCP (v3.3) for Adobe AI Coworker and Cursor: lab execution framework + per-industry playbooks (resources + tools), list industries/sandboxes, profile infra, generate stream test profiles (7-industry persona parity + segment_hint packs), batch seed, get/lookup/update UPS (full-snapshot stitch), profile activity, events, and sandbox onboarding — all with X-AEP-Lab-Mcp-Key.',
      useCases: [
        'Call lab_get_execution_framework / lab_get_industry_playbook first — lab conventions without manual retraining',
      ...
      configNotes:
        'Cursor or Coworker ~/.cursor/mcp.json: "type": "streamable-http", url https://aep-lab-profile-mcp-109406613852.us-central1.run.app/mcp, headers X-AEP-Lab-Mcp-Key: <your key>. Sign in on MCP servers page (Profile Viewer → MCP servers) to generate, rotate, or revoke a personal key scoped to your workspace sandbox — or use the shared ops key from GCP Secret Manager. Never commit secrets. Framework: lab_get_execution_framework, lab_get_industry_playbook, resources lab://framework/*. Tools: lab_list_industries, lab_list_sandboxes, lab_mcp_access_info, lab_profile_infra_status, lab_generate_profile, lab_lookup_profile, lab_get_profile, lab_update_profile, lab_profile_activity, lab_list_event_targets, lab_send_profile_event, lab_send_edge_event, lab_sandbox_profile_config, lab_onboard_sandbox, lab_generate_profiles_batch, lab_batch_job_status, lab_provision_profile_infra_step, lab_enable_profile. Long-running tools — MCP client timeout ≥ 300s. Verify allowlist with lab_mcp_access_info.',
      docUrl:
