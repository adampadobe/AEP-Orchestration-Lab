...
(function (global) {
  ...
  function generateScrapeBrief(opts) {
    ...
    return [
      ...
      '5. Return to **Brand Scraper → Scrape blocked? Offline fallback**, drop the `.zip` in the upload box.',
      '6. **Uploaded HTML only** is enabled automatically when you drop files there; otherwise use **Options → HTML upload** and tick **Uploaded HTML only** if the site cannot be crawled at all.',
      ...
      '```',
      '',
      'Drop the ZIP in **Scrape blocked? Offline fallback** on the Brand Scraper page (or use **Options → HTML upload**). Max **30 MB**. **Uploaded HTML only** is enabled automatically when you drop in the offline panel.',
      '',
      '---',
      '',
  ...
  function generateAssetChecklist(opts) {
    ...
    return [
      ...
      '- [ ] Dropped in Brand Scraper → **Scrape blocked? Offline fallback** (or **Options → HTML upload**)',
      '- [ ] **Uploaded HTML only** ticked if live crawl is impossible',
