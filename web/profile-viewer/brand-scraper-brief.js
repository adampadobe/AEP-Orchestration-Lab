...
(function (global) {
  ...
  function generateScrapeBrief(opts) {
    ...
    return [
      ...
      '5. Return to **Brand Scraper → Options → HTML upload**, attach the `.zip`.',
      '6. Tick **Uploaded HTML only** if the site cannot be crawled at all; otherwise leave **Use uploaded HTML as fallback** on and run **Analyse**.',
      ...
      '```',
      '',
      'Upload via **Options → HTML upload** on the Brand Scraper page. Max **30 MB**. Then run **Analyse** with **Uploaded HTML only** checked.',
      '',
      '---',
      '',
  ...
  function generateAssetChecklist(opts) {
    ...
    return [
      ...
      '- [ ] Uploaded in Brand Scraper → **Options → HTML upload**',
