...
(function attachHomeNewGreeting(global) {
  ...
  function migrateLegacyCanvasTheme() {
    try {
      var legacy = localStorage.getItem('aepHomeNewCanvas');
      if (legacy === 'light' && localStorage.getItem('aepTheme') === 'dark') {
        localStorage.setItem('aepTheme', 'light');
        if (global.AepTheme && typeof global.AepTheme.applyToDocument === 'function') {
          global.AepTheme.applyToDocument('light');
        } else {
          document.documentElement.removeAttribute('data-aep-theme');
        }
      }
      if (legacy != null) localStorage.removeItem('aepHomeNewCanvas');
    } catch (_e) {}
  }
