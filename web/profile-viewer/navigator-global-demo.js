...
(function (global) {
  ...
  function run() {
    ...
    async function loadGeneratorTargets() {
      ...
      if (typeof window.AepDemoGeneratorTargets !== 'undefined' && window.AepDemoGeneratorTargets.loadGeneratorTargetsIntoSelect) {
        generatorTargets = await window.AepDemoGeneratorTargets.loadGeneratorTargetsIntoSelect(selectEl, {
          preferredId: NAVIGATOR_PRESET_ID,
        });
        return;
      }
      try {
        ...
        if (generatorTargetSelect.querySelector('option[value="' + NAVIGATOR_PRESET_ID + '"]')) {
          selectEl.value = NAVIGATOR_PRESET_ID;
        }
      } catch {
        selectEl.innerHTML = '';
        const opt = document.createElement('option');
    ...
    if (navigatorTagsInjection && window.envBar && typeof window.envBar.registerTagsInjection === 'function') {
      window.envBar.registerTagsInjection(navigatorTagsInjection);
    }
