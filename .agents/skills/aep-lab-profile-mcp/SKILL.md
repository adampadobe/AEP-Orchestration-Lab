...
description: >-
  ...
  (Streamable HTTP on Cloud Run v3.43.0). Use when choosing an MCP context, generating test profiles, sending
...
MCP server: **AEP Orchestration Lab MCP v3.43.0** (`aep-orchestration-lab-mcp`; see `tools/aep-lab-profile-mcp/README.md`).
...
**One-click Coworker install:** `tools/coworker-marketplace/` packages all eleven focused endpoints, including read-only Commerce at `/mcp/commerce` and Commerce Optimizer at `/mcp/commerce-optimizer`, into one installable Coworker Marketplace plugin, avoiding the need to add each connection by hand. See `tools/coworker-marketplace/README.md`.
...
**Coworker:** install the marketplace plugin and remain signed in with Adobe IMS. Create a Portal key once to establish sandbox enrollment, but do not paste its plaintext into Coworker.
...
**Focused Coworker config:** name `aep-lab-demo-prep`, URL ending `/mcp/demo-prep`, using the marketplace plugin's Adobe IMS passthrough provider.

**Focused PDF Coworker config:** name `aep-lab-pdf-prep`, URL ending `/mcp/pdf`, using the marketplace plugin's Adobe IMS passthrough provider. See `docs/COWORKER_PDF_PREP_MCP.md` for copy-ready workflows, limits, and prompts.
