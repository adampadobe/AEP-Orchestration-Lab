...
test('context directory exposes copy-ready unique names and URLs', () => {
  ...
  assert.equal(contexts.find((context) => context.id === 'aep-lab-general').toolCount, 202);
  ...
  assert.equal(contexts.find((context) => context.id === 'aep-lab-creativity').url.endsWith('/mcp/creativity'), true);
  assert.equal(contexts.find((context) => context.id === 'aep-lab-creativity').toolCount, 16);
...
test('recommender selects narrow contexts and reports cross-context work', () => {
  ...
  assert.equal(recommendMcpContexts('apply Lightroom edits in Photoshop').primary.id, 'aep-lab-creativity');
...
test('workflow plans retain confirmation gates', () => {
  ...
  assert.deepEqual(getMcpWorkflow('creative_asset_production').contexts, ['aep-lab-creativity']);
...
test('Profile Viewer MCP page matches the deployed catalog and separates Coworker from key-based setup', () => {
  ...
  assert.match(html, /v3\.50\.0/);
  assert.match(html, /installs General plus fifteen focused integrations/i);
  ...
  assert.match(catalog, /Complete Lab MCP · 202 tools/);
  ...
  assert.match(catalog, /id: 'aep-lab-creativity'/);
  assert.match(catalog, /\/mcp\/creativity/);
