...
`lab_weather_map` looks up current weather (same params as above, plus optional `zoom` 1-20) and additionally renders a Google Static Maps image with a marker at that location. The map is returned two ways in the same response: as an embedded MCP image content block, and as a Markdown image link (`![...](https://storage.googleapis.com/...)`) pointing to a copy uploaded to the dedicated `aep-orchestration-lab-weather-maps` GCS bucket — object names are random and unguessable, the bucket grants object-level public read only (no listing), and every object auto-deletes after 1 day. The two forms exist because some MCP clients (e.g. Adobe Coworker, as of testing) render a Markdown image URL in chat but don't yet render inline MCP image content blocks — the tool description tells the model to display the Markdown link either way. Requires `GOOGLE_MAPS_API_KEY`, restricted to the Static Maps and Geocoding APIs only. The raw Static Maps request itself never returns a URL to the caller, since that URL carries the API key as a query param — only the hosted copy's URL (which carries no API key) is exposed.
