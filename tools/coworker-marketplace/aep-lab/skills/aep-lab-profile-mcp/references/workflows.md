# AEP Orchestration Lab MCP — full workflow playbook
...
3. **Confirm and activate**
   ...
   > After explicit colleague confirmation, call **lab_demo_customer_switch** with the returned `asset_preflight_id` and `config_preflight_id`, `confirmed:true`, and one stable `idempotency_key`. Only report ready when `verification.readyToPresent` is true.
...
> After connecting, call **lab_mcp_first_run_setup** for sandbox **prisacar** with **workspace_slug** **prisacar**, **first_name** Priya, **last_name** Sacar, **adobe_email** prisacar@adobe.com. Report what is ready vs needs Portal or **lab_onboard_sandbox**.
...
## Workflow 1b — Snowflake readiness + dual-load travel
...
## Workflow 1c — Snowflake travel governed provision
...
## Workflow 1d — Snowflake non-travel event enrichment
...
## Workflow 1e — AJO Live Activity customer test
...
## Workflow 2b — FSI segment personas
...
## Workflow 2c — Retail segment personas
...
When switching to a sandbox that has no Firestore connection docs, generate/update will fail until infra is provisioned.
...
4. **Execute all industries (async)**
...
5. **Ops note for new colleague sandboxes**
   ...
   > Ops seeds Firestore mcpSandboxAllowlist/{keyId} or updates AEP_LAB_MCP_ALLOWED_SANDBOXES — see README. Verify with lab_mcp_access_info.
...
**Chain when a colleague reports missing events:**
...
## Workflow 5d — Decisioning lab Edge evaluate
...
## Workflow 5e — Decisioning catalog browse + health
...
## Workflow 5f — Governed audience audit + deletion
...
1. **List/search (read-only)**
   ...
   > **lab_audience_list** sandbox apalmer name "demo" — inspect the exact `id`, name, origin, lifecycle, and timestamps.
