...
description: >-
  ...
  (Streamable HTTP on Cloud Run v3.48.0). Use when choosing an MCP context, generating test profiles, sending
...
18. **Firefly generation is capabilities → preview → exact confirmation → one submit → status** — use **`lab_firefly_capabilities`**, then the image or video preview tool. Show the request, preflight ID, and billable warning; call the matching apply tool only with the unchanged request and exact confirmation. Never automatically retry a submit because Firefly generation is billable and non-idempotent. Poll both media types with **`lab_firefly_job_status`** and cancel only with **`lab_firefly_job_cancel`** and its exact job confirmation. Firefly credentials are server-side; never ask the colleague for a client secret.
