#!/usr/bin/env node
/**
 * POST equivalent of Event Tool → "Add booker/stayer field group":
 * creates tenant field group "AEP Event Tool - Booker Stayer v1" (if missing)
 * and attaches it to the Event Tool schema.
 *
 * Loads ~/.config/adobe-ims/credentials.env without overwriting non-empty env vars.
 *
 * Usage:
 *   node scripts/ensure-booker-stayer-fieldgroup.cjs "https://ns.adobe.com/demoemea/schemas/…" [sandboxName]
 */
...
async function main() {
  ...
  const result = await eventInfra.runEventInfraStep(sandbox, token, clientId, orgId, 'ensureBookerStayerFieldGroup', {
    schemaId,
  });
