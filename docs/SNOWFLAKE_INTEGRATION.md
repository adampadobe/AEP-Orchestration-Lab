...
Snowflake credentials are **per Firebase uid + sandbox** — the same scoping as
the Profile Viewer UI. Coworker must use a **user-generated MCP API key**
(Profile Viewer → **MCP servers**), not the shared ops/env key. The ops key
has no `principalUid`, so Snowflake tools return `MCP_USER_KEY_REQUIRED`.
...
| UI shows saved config; MCP shows empty / no credential | **Uid mismatch:** MCP key was generated by a different Firebase user than the one who saved Snowflake in Profile Viewer. Regenerate the MCP key while signed in as the same user, or re-save Snowflake under the MCP key's `principalUid`. |
