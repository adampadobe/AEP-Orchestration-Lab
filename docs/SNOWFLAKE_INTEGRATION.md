...
## Per-user and sandbox-shared configuration

Connection details are scoped to **(lab user, AEP sandbox)** by default so multiple
people can use different Snowflake targets without overwriting each other.

On **team sandboxes** (technical names containing `apalmer` or `kirkham`), a
**sandbox-shared** credential is also stored so any lab user on that sandbox —
including a new browser with a different anonymous Firebase uid — can test
connections without re-pasting the PEM.
...
Before sandbox-shared credentials, each anonymous Firebase uid had its own
Secret Manager secret — a new browser meant `hasCredential: false` even when
a teammate had already saved on the same AEP sandbox. Team sandboxes now share
...
| UI shows saved config; MCP shows empty / no credential | **Uid mismatch on non-shared sandbox:** MCP key was generated by a different Firebase user than the one who saved Snowflake. On team sandboxes, save once (dual-write) or wait for lazy migration — any uid on that sandbox should see `hasCredential: true`. |
...
The API response includes `labUserUidPrefix` (first 8 chars of the resolved
Firebase uid) so Coworker can confirm it matches the user who saved config.
