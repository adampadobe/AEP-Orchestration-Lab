...
The Profile Viewer **Event Tool** “Create schema” / **Set up event infrastructure** action (Cloud Function `eventInfraService`) creates an **ExperienceEvent** schema, attaches **Experience Event Core v2.1** when that ExperienceEvent-class field group exists in the sandbox (not Profile Core v2, which is for Profile-class schemas), auto-provisions and attaches **Interaction Details Lite** (tenant `interactionDetails.core.channel`, `deviceType`, `source`), **Travel - Hotel Experience v1**, **B2C Event Identity v1**, and **AEP Event Tool - Booker Stayer v1** when missing, and adds **non-primary** identity descriptors for **ECID** and **Email** on **`_{tenant}.identification.core.*`**. Resolution prefers the global OOTB **Interaction Details Lite** field group when present; debug titles such as **AEP Lab Test … DEBUG** are ignored. Re-run **Set up event infrastructure** or `attachRecommendedFieldGroups` to repair an existing schema — that step detaches debug Interaction Details mixins and attaches the correct field group. If a sandbox still has an old **AEP Lab Test Interaction…** mixin on the schema from earlier investigation, detach it in AEP Schemas or re-run attach after this fix ships. Then click **Enable schema & dataset for Profile (identityMap)** (or `POST /api/events/infra/step` with `step: enableForProfile`, or MCP `lab_enable_event_profile`) — same as the AEP UI **Use alternate primary identity** option: primary ID per event in **`identityMap`**, not a schema field. Send **`identityMap`** on each Edge payload as in this doc.
