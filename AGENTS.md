...
- **Dedicated Cloud Run services:** the same ship order applies. For a code-only release, inspect the current service and use `gcloud run services update --image ...` so existing environment variables, secret bindings, service account, ingress, timeout, memory, and scaling remain intact. Use `gcloud run deploy` configuration flags only for an explicitly reviewed full configuration change. Read the service back and verify the new revision, traffic, bindings, and health without printing secret values.
